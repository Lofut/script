...
UnloadButton.MouseButton1Click:Connect(function()
    ...
    for target, data in pairs(ESP) do
...
local function createESP(target)
    if ESP[target] then return ESP[target] end
...
local function setupPlayer(player)
    ...
    table.insert(Connections, player.CharacterAdded:Connect(addCharacter))
...
table.insert(Connections, workspace.DescendantRemoving:Connect(function(obj)
    if ESP[obj] and ESP[obj].IsBot then
        removeESP(obj)
    end
end))
...
table.insert(Connections, RunService.Heartbeat:Connect(function()
    ...
    for target, data in pairs(ESP) do
        ...
        -- Highlight is ON for anyone in range, regardless of walls.
        -- DepthMode.Occluded handles the "only visible pixels" rendering.
        local shouldShow = withinDistance and Settings.Thermal
        if RevealActive and withinDistance then
            shouldShow = true
        end
        ...
        local showInfo = (distance <= Settings.MaxDistance)
